...
if	[ "$?" != "0" ]; then
    ...
    docker build --no-cache -t postgres-db -f postgres.Dockerfile .
...
if	[ "$?" != "0" ]; then
    cd ./application
    docker build --no-cache --build-arg JAR_FILE=/sprinbgoot/target/SimpleApplication-1.0.0.0.jar -t springboot-app .
    cd ..
fi;
